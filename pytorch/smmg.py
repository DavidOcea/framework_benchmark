...
class Solver(object):
    def __init__(self, config):
        self.model = None
        self.gpu_devices = ','.join([str(id) for id in config.gpu_devices])
    ...
    def load_data(self):
        ...
        self.train_loader = torch.utils.data.DataLoader(dataset=train_set, batch_size=self.train_batch_size, shuffle=True, num_workers=self.num_worker)
        test_set = torchvision.datasets.CIFAR10(root='../data', train=False, download=True, transform=test_transform)
        self.test_loader = torch.utils.data.DataLoader(dataset=test_set, batch_size=self.test_batch_size, shuffle=False, num_workers=self.num_worker)
    ...
    def load_model(self):
        ...
        self.model = nn.DataParallel(WideResNet(depth=28, num_classes=10)).to(self.device)
